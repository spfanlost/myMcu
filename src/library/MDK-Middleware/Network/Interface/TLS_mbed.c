...
/* Configure mbedTLS debug */
  ...
  ret = mbedtls_pk_parse_key (&pkey_srv,  NetSecurity_ServerKey,
                                          NetSecurity_ServerKey_Len, NULL, 0);
  if (ret != 0) {
  ...
  ret = mbedtls_x509_crt_parse (&cacert, (const uint8_t *)mbedtls_test_cas_pem,
                                mbedtls_test_cas_pem_len );
  if (ret != 0) {
    /* Trusted CA certificate(s) parse failed */
    return (ret);
  }
#endif
  ...
  for (i = 0, tls_s = &tls_scb[0]; i < TLS_NUM_SESS; tls_s++, i++) {
    switch (tls_s->State) {
      ...
      case TLS_STATE_SETUP:
        /* Socket is connected, now start SSL handshaking */
        tls_s->CloseRq   = 0;
        tls_s->ssl_error = 0;
        mbedtls_ssl_init (tls_s->ssl);
...
#if 0
        if (tls_s->srv_name) {
          /* Verify the server certificate for client mode */
          if (mbedtls_ssl_get_verify_result (tls_s->ssl) != 0) {
            tls_s->ssl_error = MBEDTLS_ERR_SSL_PEER_VERIFY_FAILED;
            goto close;
          }
        }
#endif
        ...
        if (tls_s->t_buf != NULL) {
          ...
          if (ret > 0) {
            ...
            if (tbuf->index < tbuf->length) {
              /* Buffer not completed, try again later */
              if (tls_s->CloseRq) {
                break;
              }
            }
