...
struct bsp_lcd * lcd_bsp = &bsp_myf407_lcd;
...
{
    lcd_bsp->write_cmd(regval);
}
//写LCD数据
//data:要写入的值
void LCD_WR_DATA(vu16 data)
{
    lcd_bsp->write_data((byte_t*)&data, 1);
}
//读LCD数据
//返回值:读到的值
...
{
	vu16 ram;			//防止被优化
    lcd_bsp->read_data((byte_t*)&ram, 1);
	return ram;
}
//写寄存器
//LCD_Reg:寄存器地址
//LCD_RegValue:要写入的数据
void LCD_WriteReg(word_t LCD_Reg,word_t LCD_RegValue)
{
    lcd_bsp->write_cmd(LCD_Reg);
    lcd_bsp->write_data((byte_t*)&LCD_RegValue, 1);
}
//读寄存器
//LCD_Reg:寄存器地址
//返回值:读到的数据
u16 LCD_ReadReg(u16 LCD_Reg)
{
    word_t LCD_RegValue;
    lcd_bsp->write_cmd(LCD_Reg);
    lcd_bsp->read_data((byte_t*)&LCD_RegValue, 1);
	delay_us(5);
	return LCD_RegValue;		//返回读到的值
}
//开始写GRAM
void LCD_WriteRAM_Prepare(void)
{
    lcd_bsp->write_cmd(lcddev.wramcmd);
}
//LCD写GRAM
//RGB_Code:颜色值
void LCD_WriteRAM(u16 RGB_Code)
{
	lcd_bsp->write_data((byte_t*)&RGB_Code, 1);
}
