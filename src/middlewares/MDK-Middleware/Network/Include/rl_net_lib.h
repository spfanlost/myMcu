...
typedef struct net_ip4_cfg {
  ...
  uint8_t      NoEcho  :1;              ///< Disable ping/echo response
...
/// IP4-Frag Configuration info
typedef struct net_ip4_frag_cfg {
  NET_IP4_FRAG_INFO *Scb;               ///< Reassembly control block array
...
typedef struct {
  ...
  uint32_t(*add_line)  (void*,char*);   ///< Add authentication header line
...
typedef struct net_http_cfg {
  ...
  uint8_t   NonceTout;                  ///< Cache entry timeout in seconds
